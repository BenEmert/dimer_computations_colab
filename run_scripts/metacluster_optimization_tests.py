...
## Settings for TuneK
parser1.add_argument('--base_dir', default='../results/true_inference/v3_ftol-10', type=str) # base directory for output
...
def do_opt(opt_setts, foo, seed=None):
    ...
    analyzeOpt = AnalyzePymoo(res, foo.Knames)
    analyzeOpt.make_plots(foo.output_dir)
    return res

def opt_wrapper(opt_setts, n_targets=10):
    ...
    for c in range(n_targets):
        ...
        ip = np.random.randint(Ksim.shape[0])
        iw = np.random.randint(weights.shape[0])
        print('ip', ip)
        print('iw', iw)

        K = Ksim[ip] # binding affinity
        a0 = a0sim[ip] # accessory monomer concentration
        theta = weights[iw] # dimer weights
        ...
        FOO.loss_k(K, final_run=True, plot_surface=True)
