...
## Settings for TuneK
parser1.add_argument('--base_dir', default='../results/bumps/a-inner_w-outer/', type=str) # base directory for output
...
parser1.add_argument('--w_opt', default="outer", type=str)
...
parser1.add_argument('--dimer_eps', default=1e-16, type=float)
...
def opt_wrapper(opt_setts, n_rounds=10, bump_center=0.5, bump_width=2, **kwargs):
    ...
    # here is a K that achieves nices bumps
    K = np.array([ 0.75394773, -0.52364236,  2.58714428, -1.53164927,  4.07253698,  0.42866631])
    a0 = np.array([[-2.81203886, -1.19354927]]).T

    FOO.truth = {'K': np.atleast_2d(K), 'a0': a0, 'theta': []}
    ...
    FOO.dimer_eps = 1e-16
    FOO.loss_k(K, n_starts=2, final_run=True, plot_surface=True, extra_nm='TrueK_eps1e-16')

    FOO.dimer_eps = 1e-8
    FOO.loss_k(K, n_starts=2, final_run=True, plot_surface=True, extra_nm='TrueK_eps1e-8')
